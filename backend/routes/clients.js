...
router.post('/', async (req, res) => {
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`,
            [name, phone, address, neighborhood, geo_lat, geo_lng, fixed_price, payment_due_day, last_sand_change]
        );
...
router.put('/:id', async (req, res) => {
    try {
       ...
       WHERE id = $10 RETURNING *`,
            [name, phone, address, neighborhood, geo_lat, geo_lng, fixed_price, payment_due_day, last_sand_change, id]
        );
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Client not found' });
        }
